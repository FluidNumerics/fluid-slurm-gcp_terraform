...
locals {
  ...
  posix_username = replace( replace(var.email_address,"@","_"),".","_" )
  slurm_accounts = [{"name"="wrf-users","users"=[local.posix_username],"allowed_partitions"=["all"]}]
}
...
module "slurm_gcp" {
  ...
  slurm_gcp_admins = ["user:${var.email_address}"]
  slurm_gcp_users = ["user:${var.email_address}"]
  name = var.cluster_name
  tags = [var.cluster_name]
  controller = local.controller
  login = local.login
  partitions = local.partitions
  slurm_accounts = local.slurm_accounts
  mounts = []
}
